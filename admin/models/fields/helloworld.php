...
class JFormFieldModal_Helloworld extends JFormField
{
    ...
    {
        // Load language
        JFactory::getLanguage()->load('com_testing', JPATH_ADMINISTRATOR);
        ...
        if ($value)
        {
            ...
            $query = $db->getQuery(true)
                ->select($db->quoteName('name')) //greeting
                ->from($db->quoteName('#__test_documents'))
        ...
        // url for the iframe
        $linkHelloworlds = 'index.php?option=com_testing&amp;view=helloworlds&amp;layout=modal&amp;tmpl=component&amp;' . JSession::getFormToken() . '=1';
